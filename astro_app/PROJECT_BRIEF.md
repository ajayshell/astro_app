...
**New page: Jamakol Chart** (`web/src/astro/jamakol.ts`, `web/src/pages/JamakolPage.tsx`). Divides the day (sunrise to next sunrise) into 8 jamams — 4 across daytime, 4 across the night — drawn as a ring of 8 boxes around a 3x3 grid (center box shows date/time/place), read anti-clockwise starting from the top-left box, matching the reference screenshot supplied. Has its own date/time/place inputs plus a "use current time" button, and defaults to the current moment at Bengaluru on load. The jamam containing the selected moment is highlighted.
