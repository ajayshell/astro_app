...
**New page: Jamakol Chart** (`web/src/astro/jamakol.ts`, `web/src/pages/JamakolPage.tsx`). Divides the day (sunrise to next sunrise) into 8 jamams — 4 across daytime, 4 across the night — drawn as an outer ring of 8 boxes (the 4 corners + 4 edge positions of a 6x6 grid) surrounding a separate inner 4x4 area, with a 2x2 center block showing date/time/place. The inner area is currently blank placeholder cells — traced from the reference screenshot, it looks like it holds its own South-Indian-chart-style sub-details (nakshatra sub-positions, Lagna, etc.) that I don't have rules for yet, so it's left empty rather than guessed. The ring is read anti-clockwise starting from the top-left box. Has its own date/time/place inputs plus a "use current time" button, defaults to the current moment at Bengaluru on load, and highlights the jamam containing the selected moment.
...
- **Not confirmed — flagged with a warning banner on the page itself:**
  ...
  2. *Maandi's position.* Maandi/Gulika is normally a real calculated upagraha (a time-of-day-dependent point), not just a label. Right now it's placed as the 8th ring slot with no astronomical computation behind it.
  3. *The meaning of the "degree" shown in each box.* The reference screenshot shows values like "315.3", "0.3", "45.3" — 45-degree steps with a mysterious ".3". I don't know what that fractional part represents (a real ephemeris degree? a rounding artifact? something else?), so this build just shows plain 45-degree-step labels (0°, 45°, 90°...) with no claim they mean the same thing as the reference app's numbers.
  4. *All the other cells in the reference screenshot* (e.g. "ஆரூ-23", "ரா-8", "லக்-11" and similar) aren't implemented — I only built the 8 ring boxes plus the center caption, since I don't yet know what those other cells represent. Ready to add once you describe them.
